...
{
    public:
        BULKIO_dataUshort_Out_i(std::string port_name, SourceVITA49_base *_parent);
        ...
        void pushPacket(unsigned short* data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID, int size) {
                    if (refreshSRI) {
                        if (currentSRIs.find(streamID) == currentSRIs.end()) {
                            BULKIO::StreamSRI sri;
                            sri.hversion = 1;
                    ...
                    // as the data for the sequence.  The 'false' at the end is whether or not
                    // CORBA is allowed to delete the buffer when the sequence is destroyed.
                    PortTypes::UshortSequence seq = PortTypes::UshortSequence(size, size, &(data[0]), false);
                    if (active) {
                        std::vector < std::pair < BULKIO::dataUshort_var, std::string > >::iterator port;
                        for (port = outConnections.begin(); port != outConnections.end(); port++) {
                            try {
                                ((*port).first)->pushPacket(seq, T, EOS, streamID.c_str());
                                stats[(*port).second].update(size, 0, 0, streamID);
                            } catch(...) {
                                std::cout << "Call to pushPacket by BULKIO_dataUshort_Out_i failed" << std::endl;
                            }
                        }
...
{
    public:
        BULKIO_dataShort_Out_i(std::string port_name, SourceVITA49_base *_parent);
        ...
        void pushPacket(short* data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID, int size) {
                   if (refreshSRI) {
                       if (currentSRIs.find(streamID) == currentSRIs.end()) {
                           BULKIO::StreamSRI sri;
                           sri.hversion = 1;
                   ...
                   // as the data for the sequence.  The 'false' at the end is whether or not
                   // CORBA is allowed to delete the buffer when the sequence is destroyed.
                   PortTypes::ShortSequence seq = PortTypes::ShortSequence(size, size, &(data[0]), false);
                   if (active) {
                       std::vector < std::pair < BULKIO::dataShort_var, std::string > >::iterator port;
                       for (port = outConnections.begin(); port != outConnections.end(); port++) {
                           try {
...
{
    public:
        BULKIO_dataChar_Out_i(std::string port_name, SourceVITA49_base *_parent);
        ...
        void pushPacket(char* data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID, int size) {
            if (refreshSRI) {
                if (currentSRIs.find(streamID) == currentSRIs.end()) {
                    BULKIO::StreamSRI sri;
                    sri.hversion = 1;
            ...
            // as the data for the sequence.  The 'false' at the end is whether or not
            // CORBA is allowed to delete the buffer when the sequence is destroyed.
            PortTypes::CharSequence seq = PortTypes::CharSequence(size, size, (CORBA::Char*)&(data[0]), false);
            if (active) {
                std::vector < std::pair < BULKIO::dataChar_var, std::string > >::iterator port;
                for (port = outConnections.begin(); port != outConnections.end(); port++) {
                    try {
...
{
    public:
        BULKIO_dataDouble_Out_i(std::string port_name, SourceVITA49_base *_parent);
...
{
    public:
        BULKIO_dataFloat_Out_i(std::string port_name, SourceVITA49_base *_parent);
        ...
        void pushPacket(float* data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID, int size) {
            if (refreshSRI) {
                if (currentSRIs.find(streamID) == currentSRIs.end()) {
                    BULKIO::StreamSRI sri;
                    sri.hversion = 1;
            ...
            // as the data for the sequence.  The 'false' at the end is whether or not
            // CORBA is allowed to delete the buffer when the sequence is destroyed.
            PortTypes::FloatSequence seq = PortTypes::FloatSequence(size, size, &(data[0]), false);
            if (active) {
                std::vector < std::pair < BULKIO::dataFloat_var, std::string > >::iterator port;
                for (port = outConnections.begin(); port != outConnections.end(); port++) {
                    try {
...
{
    public:
        BULKIO_dataOctet_Out_i(std::string port_name, SourceVITA49_base *_parent);
        ...
        void pushPacket(unsigned char* data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID, int size) {
                   if (refreshSRI) {
                       if (currentSRIs.find(streamID) == currentSRIs.end()) {
                           BULKIO::StreamSRI sri;
                           sri.hversion = 1;
                   ...
                   // as the data for the sequence.  The 'false' at the end is whether or not
                   // CORBA is allowed to delete the buffer when the sequence is destroyed.
                   CF::OctetSequence seq = CF::OctetSequence(size, size, &(data[0]), false);
                   if (active) {
                       std::vector < std::pair < BULKIO::dataOctet_var, std::string > >::iterator port;
                       for (port = outConnections.begin(); port != outConnections.end(); port++) {
                           try {
...
// ----------------------------------------------------------------------------------------
// BULKIO_dataVITA49_In_i declaration
// ----------------------------------------------------------------------------------------
class BULKIO_dataVITA49_In_i : public POA_BULKIO::dataVITA49, public Port_Provides_base_impl
{
    public:
        BULKIO_dataVITA49_In_i(std::string port_name, SourceVITA49_base *_parent);
        ~BULKIO_dataVITA49_In_i();
    ...
    protected:
        ...
        std::map<std::string, BULKIO::VITA49StreamDefinition*> attachedStreamMap;
